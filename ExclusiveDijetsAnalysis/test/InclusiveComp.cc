...
void InclusiveComp::LoadFile(std::string fileinput, std::string processinput){
...
void InclusiveComp::Run(std::string filein_, std::string savehistofile_, std::string processname_, std::string filecor_, std::string hname_, double jet1PT_, double jet2PT_, int optnVertex_, int optTrigger_, bool switchWeightPU_, bool switchWeightLumi_, bool switchWeightEff_, bool switchWeightePw_, bool switchMultiple_, bool switchPreSel_, bool switchTrigger_, bool switchVertex_, double weightlumipass_){
   ...
   if (check4.IsZombie()){
      ...
      std::cout << "\n--------------------------------------------------" << std::endl;
      std::cout << " There is no file with Efficiency corrections or the"   << std::endl;
      std::cout << " path is not correct." << std::endl;
      std::cout << " Edit the source and recompile." << std::endl;
      std::cout << "--------------------------------------------------" << std::endl;
      return;
   ...
   TH1F* h_eff = (TH1F*)l1->Get(hname.c_str());
...
*/  
  ...
  for(int i=0;i<NEVENTS;i++) {
    ...
    if (!switchMultiple || (switchMultiple && eventexcl->GetNPileUpBx0() < 21)){    
     ...
     outstring << " " << std::endl;
     outstring << " " << std::endl;
     outstring << "Input Correction file: " << filecor << std::endl;
     outstring << "Histo Name: " << hname << std::endl;
     outstring << " " << std::endl;
...
int main(int argc, char **argv)
{
   ...
   if (argc > 5 && strcmp(s1,argv[5]) != 0)  hname_  = argv[5];
   ...
   if (argc > 13 && strcmp(s1,argv[13]) != 0)  switchWeightePw_   = atoi(argv[13]);
   if (argc > 14 && strcmp(s1,argv[14]) != 0)  switchMultiple_   = atoi(argv[14]);
   if (argc > 15 && strcmp(s1,argv[15]) != 0)  switchPreSel_   = atoi(argv[15]);
   if (argc > 16 && strcmp(s1,argv[16]) != 0)  switchTrigger_   = atoi(argv[16]);
   if (argc > 17 && strcmp(s1,argv[17]) != 0)  switchVertex_   = atoi(argv[17]);
   if (argc > 18 && strcmp(s1,argv[18]) != 0)  weightlumipass_  = atof(argv[18]);

   InclusiveComp* IncluDijets = new InclusiveComp();   
   IncluDijets->Run(filein_, savehistofile_, processname_, filecor_, hname_, jet1PT_, jet2PT_, optnVertex_, optTrigger_, switchWeightPU_, switchWeightLumi_, switchWeightEff_, switchWeightePw_, switchMultiple_, switchPreSel_, switchTrigger_, switchVertex_, weightlumipass_);

...
void DiffractiveZComp::Run(std::string filein_, std::string ttreename_, std::string savehistofile_, bool switchTrigger_, int optTrigger_, bool switchPreSel_, int nVertex_, bool switchPUMultiple_, bool switchmcweight_, float mcweight_){
  ...
  // Reco Variables
  double aSumE = 0.;
  double deltaphielectrons = 0;
  double deltaphimuons = 0.;
  ...
  for (int j=0; j<3; j++){
	...
	sprintf(tag,"%i",k);
      ...
      TH1D *histo_deltaphielectrons = new TH1D(name33,"Number of Vertex; # Vertex; N events",25,0,25);
      ...
      TH1D *histo_deltaphimuons = new TH1D(name34,"Number of Vertex; # Vertex; N events",25,0,25);

...
#include "DiffractiveZExclusive.h"
...
void DiffractiveZExclusive::LoadFile(std::string fileinput, std::string processinput){
...
void DiffractiveZExclusive::Run(std::string filein_, std::string ttreename_, std::string savehistofile_, bool switchTrigger_, int optTrigger_, bool switchPreSel_, double lepton1pt_, double lepton2pt_, int nVertex_, bool switchPUMultiple_, bool switchmcweight_, float mcweight_){
  ...
  for (int j=0; j<7; j++){
	...
	sprintf(tag,"%i",k);
      ...
      char name35[300];
      sprintf(name35,"VertexVsLuminosity_PU_%s_%s",tag,Folders.at(j).c_str());
...
int main(int argc, char **argv){
  ...
  DiffractiveZExclusive* diffZRun = new DiffractiveZExclusive();   

...
void EffMacroFWDTrigger::LoadFile(std::string fileinput, std::string processinput){
...
void EffMacroFWDTrigger::Run(std::string filein_, std::string savehistofile_, std::string processname_, double jet1PT_, double jet2PT_, int optnVertex_, int triggereffpass_, double sumEHFPlus_, double sumEHFMinus_, int optTrigger_, bool switchSelJet1Eff_, bool switchSelJet2Eff_, bool switchSelDijetsEff_, bool switchSelHFPlusEff_, bool switchSelHFMinusEff_, bool switchSelHFOREff_, bool switchSelHFANDEff_){
 ...
 TFile check1(filein.c_str());
  ...
  //--------------------------------------------------------------------------------------------------------------------------
   for(int i=0;i<NEVENTS;i++) {
     ...
     //int triggerRef = eventexcl->GetHLTPath(triggereffpass); 
     //int triggerAna = eventexcl->GetHLTPath(optTrigger);      
 
     double lumiBx = eventexcl->GetInstLumiBunch();
     double sumEnergyHFPlus = eventdiff->GetSumEnergyHFPlus();
     double sumEnergyHFMinus = eventdiff->GetSumEnergyHFMinus();
     double JetPt1 = eventexcl->GetLeadingJetPt();
     double JetPt2 = eventexcl->GetSecondJetPt();
     double JetPt3 = eventexcl->GetThirdJetPt();
     ...
     ////////////////////////////////////////////
        ...
        //if(triggerRef == 1 ){
        if(eventexcl->GetHLTPath(triggereffpass)){
         ++nAll;
            ...
            if(switchSelHFANDEff && ((sumEnergyHFPlus < sumEHFPlus)&&(sumEnergyHFMinus < sumEHFMinus))){
              ...
              if(selDijets && !selJet1 && !selJet2){ 
                if(selHFPlus && !selHFMinus && !selHFAND && !selHFOR)
                   ...
                   histosAll[2]->Fill(lumiBx);
                   histosAll[3]->Fill(JetPt3);  
              }
              else if(!selDijets && selJet1 && !selJet2) { 
                if(!selHFPlus && !selHFMinus && selHFAND && !selHFOR)
                   histosAll[4]->Fill(JetPt1);}
              else if(!selDijets && !selJet1 && selJet2) {
                if(!selHFPlus && !selHFMinus && selHFAND && !selHFOR)
                   histosAll[5]->Fill(JetPt2);}
              ...
              if(eventexcl->GetHLTPath(optTrigger)){
               ...
               if(selDijets && !selJet1 && !selJet2){
                if(selHFPlus && !selHFMinus && !selHFAND && !selHFOR)
                   ...
                   histosEff[2]->Fill(lumiBx);
                   histosEff[3]->Fill(JetPt3);  
              }
              else if(!selDijets && selJet1 && !selJet2) {               
                if(!selHFPlus && !selHFMinus && selHFAND && !selHFOR)
                   histosEff[4]->Fill(JetPt1);}
              else if(!selDijets && !selJet1 && selJet2) {
                if(!selHFPlus && !selHFMinus && selHFAND && !selHFOR)
                   histosEff[5]->Fill(JetPt2);}
...
int main(int argc, char **argv)
{
   ...
   if (argc > 7 && strcmp(s1,argv[7]) != 0)  triggereffpass_ = atoi(argv[7]);
   if (argc > 8 && strcmp(s1,argv[8]) != 0)  sumEHFPlus_  = atoi(argv[8]);
   if (argc > 9 && strcmp(s1,argv[9]) != 0)  sumEHFMinus_ = atoi(argv[9]);
   if (argc > 10 && strcmp(s1,argv[10]) != 0)  optTrigger_   = atoi(argv[10]);
   if (argc > 11 && strcmp(s1,argv[11]) != 0)  switchSelJet1Eff_   = atoi(argv[11]);
   if (argc > 12 && strcmp(s1,argv[12]) != 0)  switchSelJet2Eff_   = atoi(argv[12]);
   if (argc > 13 && strcmp(s1,argv[13]) != 0)  switchSelDijetsEff_   = atoi(argv[13]);
   if (argc > 14 && strcmp(s1,argv[14]) != 0)  switchSelHFPlusEff_   = atoi(argv[14]);
   if (argc > 15 && strcmp(s1,argv[15]) != 0)  switchSelHFMinusEff_   = atoi(argv[15]);
   if (argc > 16 && strcmp(s1,argv[16]) != 0)  switchSelHFOREff_   = atoi(argv[16]);
   if (argc > 17 && strcmp(s1,argv[17]) != 0)  switchSelHFANDEff_   = atoi(argv[17]);




   EffMacroFWDTrigger* exclDijets = new EffMacroFWDTrigger();   
   exclDijets->Run(filein_, savehistofile_, processname_, jet1PT_, jet2PT_, optnVertex_, triggereffpass_, sumEHFPlus_, sumEHFMinus_,optTrigger_,  switchSelJet1Eff_, switchSelJet2Eff_, switchSelDijetsEff_, switchSelHFPlusEff_, switchSelHFMinusEff_, switchSelHFOREff_, switchSelHFANDEff_);
